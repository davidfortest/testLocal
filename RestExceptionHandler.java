 ...
 * </tr>
 * </table>
 * <h2>JSON Rendering</h2> This implementation comes pre-configured with
 * Spring's typical default {@link HttpMessageConverter} instances; JSON will be
 * enabled automatically if Jackson is in the classpath. If you want to match
 * the JSON representation shown in the article above (recommended) but do not
 * want to use Jackson (or the Spring's default Jackson config), you will need
 * to
 * {@link #setMessageConverters(org.springframework.http.converter.HttpMessageConverter[])
...
public class RestExceptionHandler extends AbstractHandlerExceptionResolver implements InitializingBean {
    ...
    protected ModelAndView getModelAndView(ServletWebRequest webRequest, Object handler, RestError error)
        ...
        //view
        if(error.getView() != null){
            Map<String,Object> map = new HashMap<String,Object>();
            map.put("request_uri", webRequest.getRequest().getRequestURI());
            map.put("response_message", error.getThrowable().getMessage());
            map.put("response_error", error);
            map.put("response_exception", error.getThrowable());
            map.put("request_handler", handler.getClass().getName());
            ModelAndView view = new ModelAndView(error.getView(),map);
            return view;
        }
